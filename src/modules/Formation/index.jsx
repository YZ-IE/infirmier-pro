...
const TOOLS = [
  {id:'quiz',   icon:'🎯', label:'Quiz clinique',       desc:'QCM par thème · Score · Explications'},
  {id:'cas',    icon:'📖', label:'Cas cliniques',        desc:'Situations réelles · Raisonnement'},
  {id:'ia',     icon:'🤖', label:'Assistant IA',         desc:'Questions libres · Explications'},
  {id:'lexique',icon:'📚', label:'Lexique médical',      desc:'Abréviations · Définitions'},
];

export default function Formation({ onBack }) {
  const [tool,setTool]=useState(null);
  const map={quiz:<Quiz/>,cas:<CasCliniques/>,ia:<AssistantIA/>,lexique:<Lexique/>};
  if(tool) return (
  ...
  return (
    <div style={{minHeight:'100vh',background:T.bg}}>
      <div style={{background:T.formDim,borderBottom:`1px solid ${C}44`,padding:'14px 16px',display:'flex',alignItems:'center',gap:12,position:'sticky',top:0,zIndex:10}}>
        ...
        <div style={{color:T.text,fontWeight:700,fontSize:18}}>🎓 Formation</div></div>
      </div>
