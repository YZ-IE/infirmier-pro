...
export default function Iatrogenique({ onBack, initialTool = null, onFavChange }) {
  ...
  return (
    <div style={{ minHeight:'100vh', background:T.bg }}>
      ...
      <div style={{ padding:'14px', display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
        {TOOLS.map(t => (
          <div key={t.id} style={{ background:T.surface, border:`1px solid ${t.badge ? C+'66' : C+'33'}`, borderRadius:10, padding:'14px 12px', position:'relative' }}>
            {t.badge && <span style={{ position:'absolute', top:8, right:8, background:C+'22', color:C, fontSize:8, fontFamily:'monospace', padding:'2px 6px', borderRadius:8 }}>{t.badge}</span>}
      ...
      <div style={{ margin:'0 14px', background:T.iatrDim, border:`1px solid ${C}33`, borderRadius:8, padding:'10px 14px' }}>
        <div style={{ color:C, fontSize:11, fontFamily:'monospace' }}>⚠️ Toujours vérifier la prescription médicale avant toute administration.</div>
      </div>
    </div>
  );
}
