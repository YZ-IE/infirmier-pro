...
export default function Urgences({ onBack }) {
  ...
  const map = {rcp:<RCP/>,avc:<AVC/>,ana:<Anaphylaxie/>,sep:<Sepsis/>,cho:<ChocEtats/>,det:<DetressResp/>,hyp:<Hypoglycemie/>,epi:<Convulsions/>};
  if (proto) return (
  ...
  return (
    <div style={{minHeight:'100vh',background:T.bg}}>
      <div style={{background:'#431407',borderBottom:`1px solid ${C}44`,padding:'14px 16px',display:'flex',alignItems:'center',gap:12,position:'sticky',top:0,zIndex:10}}>
        ...
        <div style={{color:T.text,fontWeight:700,fontSize:18}}>🚨 Urgences</div></div>
      </div>
      <div style={{padding:'14px'}}>
        <div style={{background:C+'18',border:`1px solid ${C}44`,borderRadius:8,padding:'10px 14px',marginBottom:14}}>
          ...
          <div style={{color:T.muted,fontSize:12,marginTop:3}}>En cas d&apos;urgence réelle, appeler le médecin et le 15/18 en priorité. Ces fiches sont une aide-mémoire.</div>
        </div>
        {PROTOS.map(p=>(
          <div key={p.id} onClick={()=>setProto(p.id)} style={{...s.card,cursor:'pointer',display:'flex',alignItems:'center',gap:14,borderColor:p.urgent?C+'44':T.border}}>
